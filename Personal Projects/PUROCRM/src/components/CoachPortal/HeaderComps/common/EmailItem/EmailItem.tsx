"use client";

import React, { FC, useState } from "react";
import styles from "./EmailItem.module.css";
import { CoachInboxBarOptions } from "@/components";
import { IMAGE_URL } from "@/constants/endpoints";

interface Props {
  opened: boolean;
  profileImg: string;
  name: string;
  lastMsg: string;
  timestamp: string;
  subject: string;
}

const EmailItem: FC<Props> = ({
  opened,
  profileImg,
  name,
  lastMsg,
  timestamp,
  subject,
}) => {
  const [show, setShow] = useState(false);

  return (
    <div
      className={`${!opened ? styles.notificationItemUnopened : ""} ${
        styles.notificationItem
      }`}
    >
      <div
        className={`${
          !opened
            ? styles.notificationItemContainer
            : styles.notificationItemContainer__unopened
        }`}
      >
        <div className={styles.notificationLeft}>
          <img src={profileImg} alt="" />
          {!opened ? <p>3</p> : ""}
        </div>
        <div className={styles.notificationRightContainer}>
          <div className={styles.notificationCenter}>
            <h4>{name}</h4>
            <h5 className={`${opened ? styles.subjectOpened : ""}`}>
              {subject}
            </h5>
            <p>{lastMsg}</p>
          </div>
          <div className={styles.notificationRight}>
            <div>
              <p>{timestamp}</p>
              <img
                src={`${IMAGE_URL}/icons/dots.png`}
                alt=""
                onClick={() => setShow(true)}
              />
            </div>
          </div>
        </div>
      </div>
      {/* shows individual options for each and every email items */}
      <CoachInboxBarOptions show={show} setShow={setShow} />
    </div>
  );
};

export default EmailItem;
